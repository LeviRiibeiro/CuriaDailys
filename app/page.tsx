...
export default function HomePage() {
  ...
  return <main className="min-h-screen bg-[#fff5f1] px-3 py-4 font-sans text-[#442c28] sm:px-6 sm:py-8"><div className="mx-auto flex min-h-[calc(100vh-2rem)] max-w-[440px] flex-col overflow-hidden rounded-[2.4rem] border border-white/80 bg-[#fffaf8] shadow-[0_24px_70px_rgba(112,57,33,0.18)] sm:min-h-[780px]">
    <header className="px-6 pb-3 pt-5"><div className="mb-7 flex items-center justify-between text-xs font-semibold text-[#432e29]"><span>9:41</span><span className="max-w-[190px] truncate text-[#99766b]">Olá, {displayName}</span></div><div className="flex items-center justify-between"><div className="flex items-center gap-2"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-[#ffdfd7] text-[#f46b31]"><Sunrise size={23} strokeWidth={1.8} /></span><h1 className="font-serif text-[1.65rem] tracking-tight">Meu dia</h1></div><div className="flex gap-1"><button onClick={() => void signOut()} className="grid h-11 w-9 place-items-center text-[#936f65]" aria-label="Sair"><LogOut size={18} /></button><button onClick={() => setComposerOpen(true)} className="grid h-11 w-11 place-items-center rounded-2xl bg-[#f56b30] text-white shadow-[0_8px_15px_rgba(245,107,48,0.25)] transition hover:scale-105" aria-label="Adicionar atividade"><Plus size={23} /></button></div></div></header>
